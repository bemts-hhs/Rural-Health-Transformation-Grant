...
# and proportional weights from a standard population distribution.
#
# The function supports three options for estimating 95% confidence intervals:
#   • NULL     — No confidence intervals are calculated.
#   • "none"   — No confidence intervals are calculated.
#   • "normal" — Wald-type confidence intervals using a normal approximation.
#   • "gamma"  — Asymmetric confidence intervals using the gamma method
#                (Fay & Feuer, 1997), preferred for small sample sizes.
#
# Assumptions:
#   • The input data has already been stratified (e.g., by County and Age Group).
#   • {{ count }}, {{ local_population }}, and {{ standard_population_weight }}
#     are scalar fields within each stratum.
#   • Grouping variables (e.g., County, District) are passed to `.by` to specify
#     the resolution of aggregation for output.
#   • Rates are scaled by the `rate` argument (e.g., per 1,000; per 100,000).
#
# Inputs:
#   • data — A grouped or ungrouped data.frame or tibble with input variables.
#   • count — Unquoted column name for the event count (e.g., EMS runs).
#   • local_population — Unquoted column name for the local (stratum-specific) population.
#   • standard_population_weight — Unquoted column name for the proportional weight
#       from the reference population.
#   • .by — Optional grouping variables for final aggregation (e.g., County, District).
#   • rate — Numeric multiplier for scaling rates (default: 100,000).
#   • ci_method — One of NULL, "none", "normal", or "gamma".
#
# Output:
#   • A tibble with the following variables:
#       – Count: Total observed events per group
#       – Crude_Rate: Overall crude rate per group
#       – Age_Adjusted_Rate: Directly standardized rate per group
#       – Lower_95_CI, Upper_95_CI (if requested by ci_method)
###_____________________________________________________________________________
...
calc_age_adjusted_rate <- function(
  ...
  .by = NULL,
  rate = 100000,
  ci_method = c("none", "normal", "gamma") # choose CI method
) {
  ci_method <- match.arg(ci_method, choices = c("none", "normal", "gamma"))

  # Step 1: Calculate stratum-specific rates and variance components
  rate_data <- data |>
    dplyr::mutate(
  ...
  rate_summary <- rate_data |>
    dplyr::summarize(
      ...
      Var_AAR = sum(var_component, na.rm = TRUE),
      .by = tidyselect::all_of({{ .by }})
    )

  # Step 3: Confidence intervals (optional)
  if (ci_method != "none") {
    rate_summary <- rate_summary |>
      dplyr::rowwise() |>
      dplyr::mutate(
        Lower_95_CI = dplyr::case_when(
          ci_method == "normal" ~ Age_Adjusted_Rate - 1.96 * sqrt(Var_AAR),
          ci_method == "gamma" & Count > 0 ~
            (Age_Adjusted_Rate / (2 * Count)) *
              stats::qchisq(0.025, df = 2 * Count),
          ci_method == "gamma" & Count == 0 ~ 0
        ),
        Upper_95_CI = dplyr::case_when(
          ci_method == "normal" ~ Age_Adjusted_Rate + 1.96 * sqrt(Var_AAR),
          ci_method == "gamma" ~
            (Age_Adjusted_Rate / (2 * Count)) *
              stats::qchisq(0.975, df = (2 * Count + 2))
        )
      ) |>
      dplyr::ungroup() |>
      dplyr::select(-Var_AAR)
  } else {
    rate_summary <- rate_summary |>
      dplyr::select(-Var_AAR)
  }
